...
(function () {
  ...
  // Firefox's chrome.* compatibility namespace is callback-oriented; browser.*
  // gives the promise-based WebExtension APIs this file already expects.
  ...
  function bumpLifetime(key) {
    ...
    lifetimeTimer = setTimeout(function () {
      ...
      lifetimePending = 0;
      lifetimeTimer = null;
      runtimeApi.storage.local.get({ lifetimeFiltered: 0 }).then(function (s) {
        runtimeApi.storage.local.set({ lifetimeFiltered: s.lifetimeFiltered + add });
      });
    }, 800);
  }
  ...
  function loadSettings() {
    return runtimeApi.storage.sync.get(DEFAULTS).then(function (stored) {
  ...
  function loadCommunityList() {
    return runtimeApi.storage.local.get(["communityBrands", "remoteFlagged", "communityFetchedAt"]).then(function (c) {
        ...
        Promise.all([
          ...
          .then(function (texts) {
            ...
            if (brands.length > 1000) { // sanity check before trusting the fetch
              ...
              runtimeApi.storage.local.remove(["abfList", "abfFetchedAt"]);
  ...
  function saveUserLists() {
    runtimeApi.storage.sync.set({ allow: settings.allow, block: settings.block });
  }
  ...
  function sendReport(result, suggestion, asin) {
    ...
    fetch(REPORT_ENDPOINT + "/report", {
      ...
      body: JSON.stringify({
        ...
        asin: asin || null,
        marketplace: location.hostname,
        extVersion: runtimeApi.runtime.getManifest().version
      })
  ...
  function segControl(key, options) {
    ...
    options.forEach(function (opt) {
      ...
      b.addEventListener("click", function () {
        var patch = {};
        patch[key] = opt.value;
        runtimeApi.storage.sync.set(patch); // onChanged re-applies + re-renders
      });
      track.appendChild(b);
    });
    return track;
  ...
  function buildPanel() {
    ...
    swInput.addEventListener("change", function () {
      runtimeApi.storage.sync.set({ enabled: swInput.checked });
    });
    ...
    spInput.addEventListener("change", function () {
      runtimeApi.storage.sync.set({ hideSponsored: spInput.checked });
    });
    ...
    optLink.addEventListener("click", function () {
      var sent = runtimeApi.runtime.sendMessage({ type: "ko-open-options" });
      if (sent && typeof sent.catch === "function") sent.catch(function () {});
    });
    var version = el("span", "ko-panel-version");
    version.textContent = "v" + runtimeApi.runtime.getManifest().version;
  ...
  runtimeApi.storage.onChanged.addListener(function (changes, area) {
    if (area !== "sync") return;
  ...
  runtimeApi.runtime.onMessage.addListener(function (msg, sender, sendResponse) {
