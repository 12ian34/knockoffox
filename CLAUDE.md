...
Knockoffox is a Firefox-focused MV3 port of Knockoff that filters trademark-squat pseudo-brands out of Amazon search results. Plain classic JavaScript — **no build step, no dependencies, no frameworks, no modules**. The repo root IS the extension; load it temporarily in Firefox from `about:debugging#/runtime/this-firefox` by selecting `manifest.json`.
...
- **Manual verification:** reload the temporary add-on at `about:debugging#/runtime/this-firefox`, reload an Amazon search page. Every processed tile carries `data-ko-verdict` / `data-ko-brand` attributes; click a badge for the human-readable reason.
...
- **Cut a release:** this fork is Firefox-first. Prefer `scripts/release-firefox.sh`; treat Chrome/Safari packaging scripts as inherited upstream tooling unless the task explicitly asks for them.
